...
// 全局配置定义
sbftp_config_t config_global;

// 读取配置文件
bool sbftp_read_config()
{
#ifndef FTP_DEBUG
	const char *config_file = "/etc/sbftp.conf";
#else
	const char *config_file = "sbftp.conf";
#endif
	FILE *config_stream;
	...
	if ((config_stream = fopen(config_file, "r")) == NULL) {
		sbftp_print_info(LOG_ERR, "Open The Config File Error!");
		return false;
	}
	...
	if (config_global.ftp_path[0] == '\0') {
		sbftp_print_info(LOG_ERR, "The Config File Format Error!");
		return false;
	}
