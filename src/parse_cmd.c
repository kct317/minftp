...
// 解析读取到的内容
client_state_t sbftp_parse_cmd(user_env_t *user_env, recv_buffer &rbuff)
{
	ftp_cmd_t recv_cmd;
	int cmd_index = sbftp_anaylse_buff(user_env, &recv_cmd, rbuff);

	// 未登录的时候拒绝一切非登录指令
	if (cmd_index != 0 && cmd_index != 1 && cmd_index != 2  && user_env->is_login_in == false) {
	...
	// 已登录遭遇位置指令回复无法识别
		...
		if (!sbftp_send_client_msg(user_env->conn_fd, ftp_send_msg[FTP_E_UNKNOW_CMD])) {
			sbftp_print_info(LOG_INFO, "Write Data To Client Error!");
			return state_close;
		}
	// 请求登录或者已登录发送命令
		...
		// 指令成功执行
			...
			if (!sbftp_send_client_msg(user_env->conn_fd, ftp_send_msg[FTP_S_COMMAND])) {
				sbftp_print_info(LOG_INFO, "Write Data To Client Error!");
				return state_close;
			}
		// 权限不允许
		} else if (cmd_state == 2) {
			if (!sbftp_send_client_msg(user_env->conn_fd, ftp_send_msg[FTP_E_PERMISSION])) {
				sbftp_print_info(LOG_INFO, "Write Data To Client Error!");
				return state_close;
			}
		}
...
int sbftp_anaylse_buff(user_env_t *user_env, ftp_cmd_t *recv_cmd, recv_buffer &rbuff)
{
	...
	if (rbuff.buff[3] == ' ') {
		blank_index = 3;
	} else if (rbuff.buff[4] == ' ') {
		blank_index = 4;
	} else {
		if (rbuff.buff[3] == '\r' || rbuff.buff[3] == '\n') {
			blank_index = 3;
			has_arg = false;
		} else if (rbuff.buff[4] == '\r' || rbuff.buff[4] == '\n') {
			blank_index = 4;
			has_arg = false;
		} else {
			return -1;
	...
	// 拷贝命令
	strncpy(recv_cmd->cmd, rbuff.buff, blank_index);
	recv_cmd->cmd[blank_index] = '\0';

	if (has_arg) {
		// 拷贝参数，按照惯例支持 \r\n 和 \n
		if (rbuff.buff[rbuff.len-2] == '\r' && rbuff.buff[rbuff.len-1] == '\n') {
			strncpy(recv_cmd->arg, rbuff.buff+blank_index+1, rbuff.len-blank_index-3);
			recv_cmd->arg[rbuff.len-blank_index-3] = '\0';
		} else if (rbuff.buff[rbuff.len-1] == '\n') {
			strncpy(recv_cmd->arg, rbuff.buff+blank_index+1, rbuff.len-blank_index-2);
			recv_cmd->arg[rbuff.len-blank_index-2] = '\0';
		} else {
			// 命令没读取完，此时最好应该判断是否断包
			return -1;
		}
...
#ifdef FTP_DEBUG
	if( user_env->is_login_in == 1 )
		printf("Debug Info : Client %s Send A Command. cmd : %s  arg : %s\n", user_env->user_name, recv_cmd->cmd, recv_cmd->arg);
	else
